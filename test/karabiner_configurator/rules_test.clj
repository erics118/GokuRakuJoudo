...
(def
  result
  {:test-profile-2 [{:description  "Auto generated layer conditions"
                    ...
                    {:description  "tab-mode"
                     :manipulators [{:from       {:key_code "h"}
                                     ...
                                     :type       "basic"}
                                    {:from       {:key_code "l"}
                                     :to         [{:shell_command "/usr/local/bin/chunkc tiling::window --focus east"}]
                                     :conditions [{:name  "chunkwm-scale-mode"
                                                  ...
                                                  {:name  "chunkwm-move-mode"
                                                   ...
                                                   :type  "variable_if"}]
                                     :type       "basic"}]}]
   :Default        [{:description  "Auto generated layer conditions"
                     :manipulators [{:type            "basic"
                                     :to              [{:set_variable {:name  "chunkwm-move-mode"
                                                                       :value 1}}]
                                     :from            {:key_code "f"}
                     ...
                     :manipulators [{:from       {:key_code "i"}
                                    ...
                                    {:key  :q
                                     ...
                                     :to   [{:select_input_source {:input_mode_id   "com.googlecode.rimeime.inputmethod.Squirrel"
                                                                   ...
                                                                   :language        "zh-Hans"}}]}]}
                    {:description  "x-mode"
                     :manipulators [{:from       {:key_code "i" :modifiers {:mandatory ["left_control"]}}
                                     :to         [{:key_code "a"}]
                                     :conditions [{:name "x-mode" :value 1 :type "variable_if"}]
                                     :type       "basic"}
                                    {:parameters {:basic.simultaneous_threshold_milliseconds 250}
                                     :to         [{:set_variable {:name "x-mode" :value 1}} {:key_code "a"}]
                                     :from
                                     {:modifiers    {:mandatory ["left_control"]}
                                      ...
                                      {:detect_key_down_uninterruptedly true
                                       ...
                                       :key_up_when                     "any"
                                       :to_after_key_up
                                       [{:set_variable {:name "x-mode" :value 0}}]}}}]}
                    {:description  "y-mode"
                     :manipulators [{:from
                                     {:key_code "i"
                                      :modifiers
                                      {:mandatory ["left_command" "left_shift"] :optional ["any"]}}
                                     :to         [{:key_code "a"}]
                                     :conditions [{:name "y-mode" :value 1 :type "variable_if"}]
                                     :type       "basic"}
                                    {:parameters {:basic.simultaneous_threshold_milliseconds 250}
                                     :to         [{:set_variable {:name "y-mode" :value 1}} {:key_code "a"}]
                                     :from
                                     {:modifiers
                                      {:mandatory ["left_command" "left_shift"] :optional ["any"]}
                                      :simultaneous [{:key_code "d"} {:key_code "i"}]
                                      ...
                                      {:detect_key_down_uninterruptedly true
                                       ...
                                       :key_up_when                     "any"
                                       :to_after_key_up
                                       [{:set_variable {:name "y-mode" :value 0}}]}}}]}
                    {:description  "tab-mode"
                     :manipulators [{:from       {:key_code "h"}
                    ...
                    {:description  "any keycode"
                     :manipulators [{:from {:any "key_code"}
                                    ...
                                    {:from {:any "pointing_button"}
                                     :to   [{:key_code "a"}]
                                     :type "basic"}]}
                    {:description  "double press and held key in simlayer (to_delayed_action to_if_held_down)"
                     :manipulators [{:from       {:key_code "j"}
                                    ...
                                    {:to_if_held_down   [{:shell_command "say 'j held down'"}]
                                     :to_delayed_action {:to_if_invoked  [{:set_variable {:name  "q-mode-j-dbpress-mode"
                                                                                          :value 0}}]
                                                         :to_if_canceled [{:set_variable {:name  "q-mode-j-dbpress-mode"
                                                                                          :value 0}}]}
                                     :from              {:key_code "j"}
                                     :to                [{:shell_command "say 'j press down'"}
                                                         {:set_variable {:name  "q-mode-j-dbpress-mode"
                                                                         :value 1}}]
                                     :conditions        [{:name  "q-mode"
                                                          :value 1
                                                          :type  "variable_if"}]
                    ...
                    {:description  "more than two sim keys"
                     :manipulators [{:from {:simultaneous         [{:key_code "a"}
                                                                   {:key_code "b"}
                                                                   {:key_code "c"}]
                                            :simultaneous_options {:detect_key_down_uninterruptedly false
                                                                   :key_down_order                  "insensitive"
                    ...
                    {:description  "redefine params or condis in simlayer issue #30"
                     :manipulators [{:parameters {:basic.simultaneous_threshold_milliseconds 432}
                                     :from       {:key_code  "j"
                                                  :modifiers {:optional ["any"]}}
                                    ...
                                    {:parameters {:basic.simultaneous_threshold_milliseconds 250}
                                     ...
                                     :from       {:simultaneous         [{:key_code "d"}
                                                  ...
                                                  :simultaneous_options {:detect_key_down_uninterruptedly true
                                                                         ...
                                                                         :key_up_when                     "any"
                                                                         :to_after_key_up                 [{:set_variable {:name "vi-mode" :value 0}}]}}
                                     :conditions [{:file_paths         ["^/System/Volumes/Data/Applications/Google Chrome\\.app$"]
                                                   :bundle_identifiers ["^com\\.google\\.Chrome$"]
                                                   :type               "frontmost_application_if"}]}
                                    {:parameters {:basic.simultaneous_threshold_milliseconds 432}
                                     :from       {:key_code  "j"
                                    ...
                                    {:parameters {:basic.simultaneous_threshold_milliseconds 432}
                                     ...
                                     :conditions [{:file_paths         ["^/System/Volumes/Data/Applications/Google Chrome\\.app$"
                                                   ...
                                                   :bundle_identifiers ["^com\\.google\\.Chrome$"
                                                                        "^com\\.google\\.Chrome\\.canary$"]
                                                   :type               "frontmost_application_if"}]}]}
                    {:description  "sim pkey in from"
                     :manipulators [{:from {:simultaneous         [{:pointing_button "button5"}
  ...
  (update-conf-data
   {:profiles           {:Default
    ...
    :applications       {:safari        ["^com\\.apple\\.Safari$"]
                         :chrome        ["^com\\.google\\.Chrome$" :paths "^/System/Volumes/Data/Applications/Google Chrome\\.app$"]
                         :chrome-canary ["^com\\.google\\.Chrome\\.canary$"]
                         :chromes       ["^com\\.google\\.Chrome$" "^com\\.google\\.Chrome\\.canary$" :paths "^/System/Volumes/Data/Applications/Google Chrome\\.app$" "^/System/Volumes/Data/Applications/Google Chrome Canary\\.app$"]
                         :mono          [:paths "^\\/usr\\/local\\/bin\\/mono$"]}
    ...
    :simlayers          {:q-mode        {:key :q}
                         :x-mode        {:parameters {:basic.simultaneous_threshold_milliseconds 250},
                                         :to         [{:set ["x-mode" 1]}],
                                         :from       {:sim  [:d],
                                                      ...
                                                      :simo {:interrupt true,
                                                             ...
                                                             :afterup   {:set ["x-mode" 0]}}}}
                         :y-mode        {:parameters {:basic.simultaneous_threshold_milliseconds 250},
                                         :to         [{:set ["y-mode" 1]}],
                                         :from       {:sim  [:d],
                                                      :modi {:mandatory [:left_command :left_shift]
                                                             :optional  :any}
                                                      :simo {:interrupt true,
                                                             :dorder    :strict,
                                                             :uorder    :strict_inverse,
                                                             :afterup   {:set ["y-mode" 0]}}}}
